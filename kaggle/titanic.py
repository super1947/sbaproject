...
class Service:
    ...
    def fareBand_nominal(this) -> object:  # 요금이 다양하니 클러스터링을 하기위한 준비
        this.train = this.train.fillna(
            {'FareBand': 1})  # FareBand 는 없는 변수인데 추가함
    ...
    def accuracy_by_dtree(self, this):
        ...
        score = cross_val_score(dtree, this.train, this.label,
                                cv=Service.create_k_fold(), n_jobs=1, scoring='accuracy')
        return round(np.mean(score) * 100, 2)

    def accuracy_by_rforest(self, this):
        rforest = RandomForestClassifier()
        score = cross_val_score(rforest, this.train, this.label,
                                cv=Service.create_k_fold(), n_jobs=1, scoring='accuracy')
        return round(np.mean(score) * 100, 2)
...
class Controller:
    ...
    # machine 이 된다. 이 단계는 캐글에게 내 머신이를 보내서 평가받게 하는 것 입니다. 마치 수능장에 자식보낸 부모님 마음 ...
